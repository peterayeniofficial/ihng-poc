...
function NetworkGraph({ focusedHCPId, onSelectHCP }: HCPConnectionsMapProps) {
	...
	useEffect(() => {
		const centerId =
			focusedHCPId || (mockHCPData.length > 0 ? mockHCPData[0].id : "");
	...
	const handleEdgeHover = useCallback(
		(edgeId: string | null, connectionData?: ConnectionData | null) => {
				...
				>
					<Card className="w-80 shadow-lg border border-gray-200">
						...
						<CardContent className="space-y-1">
							...
							<p
								className="text-xs"
								aria-label={`Years of experience: ${hoveredHCP.yearsOfExperience}`}
							>
					...
					<Card className="w-80 shadow-lg border border-gray-200">
						...
						<CardContent>
							<p className="text-sm text-gray-600 mb-2">
								<strong>{hoveredConnection.source}</strong> ↔{" "}
								<strong>{hoveredConnection.target}</strong>
							</p>
...
export default function HCPConnectionsMapWrapper(
	props: HCPConnectionsMapProps,
) {
	return (
		<ReactFlowProvider>
			<NetworkGraph {...props} />
